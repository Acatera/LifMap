    ...
    internal class TerrainRenderer
    {
        internal Bitmap Render(Terrain terrain)
        {
            ...
            for (int x = 0; x < terrain.Size; x++)
            {
                ...
                {
                    var cell = terrain.GetTopCellAt(x, y);
        ...
        public Bitmap RenderSearchResults(Terrain terrain, int elevation, int depth, int materialId, int minQuality)
        {
            var image = new Bitmap(terrain.Size, terrain.Size);
            ...
            for (int x = 0; x < terrain.Size; x++)
            {
                for (int y = 0; y < terrain.Size; y++)
                {
                    ...
                    var cell = terrain.GetCellAtWithQuality(x, y, elevation, depth, materialId, minQuality);
                    if (cell.IsTopMost)
                    {
        ...
        internal Bitmap RenderLayer(Terrain terrain, int elevation, int depth)
        {
            ...
            for (int x = 0; x < terrain.Size; x++)
            {
                ...
                {
                    var color = Color.Black;
                    var cell = terrain.GetCellAt(x, y, elevation, depth);
                    if (cell.IsTopMost)
                    {
                        if (cell.Elevation < 5000)
                        {
